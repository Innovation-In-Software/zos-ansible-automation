...
Goal: Connect to z/OS and sign on to TSO using a 3270 terminal emulator.
...
Using the terminal emulator, connect to the lab z/OS instance, try your TSO id, and disconnect again. 

## Lab 2: Ping z/OS from Control Node 

Goal: Run the provided ping.yml playbook with a successful result. 

The purpose of this lab is (a) to ensure you can reach the z/OS system from the AAP server, and (b) to practice running an Ansible playbook from the command line.

## Lab 3: Explore the online documentation  

Goal: Locate the online documentation and see how it's organized.

Using a Web browser, access the Ansible-z/OS documentation at https://ibm.github.io/z_ansible_collections_doc/ and explore it a little to get a sense of how it's laid out and where to look for certain types of information. 

## Lab 4: Gather information about z/OS - Ansible from command line

Goal: Use zos_gather_facts to obtain information about the z/OS system.

In this lab, you will set up an Ansible playbook that uses the zos_gather_facts module from the ibm_zos_core collection to query the z/OS system for some basic information. The real objectives of the exercise are to get used to the documentation and how to set up an Ansible playbook that connects to z/OS and takes some action, how to check whether the action succeeded, and how to display the results. The gathered information as such is not so interesting. 

# Lab 5: Gather information about z/OS - Ansible via AAP 

Now that you have a sense of how Ansible playbooks are structured and how they run, we'll do the same exercise using the Web front-end for Ansible. 

[AAP instructions here]

# Lab 6: Allocate a QSAM data set 

Goal: Allocate a QSAM data set under your own HLQ via Ansible and AAP. 

This involves using module zos_data_set from the ibm_zos_core collection. Use the documentation to determine how to set this up.

Allocate a data set with the following attributes:
...
If the data set already exists, replace it.

[AAP instructions here]

# Lab 7: Copy local data to a QSAM data set 

Goal: Conditionally allocate a QSAM data set and copy the contents of a local file to it.

This involves using module zos_copy from the ibm_zos_core collection. Use the documentation to determine how to set this up. 

[AAP instructions here]

# Lab 8: Allocate a PDSE 

Goal: Allocate a PDSE to be used for JCL. 

Use module zos_data_set in collection ibm_zos_core.
...
# Lab 9: Copy a JCL member to the new PDSE

Goal: Copy a local file to the JCL library.

Edit local file jcl/HELLO.jcl so that the Job card is correct for your userid.

Use module zos_copy from collection ibm_zos_core to copy the contents of local file jcl/gener1.jcl to <HLQ>.LAB.JCL(HELLO).

# Lab 10: Submit JCL from a PDSE	

Goal: Submit a job via Ansible.

Use module zos_job_submit from collection ibm_zos_core to submit the JCL. Test the maximum condition code. Display the contents of SYSUT2.

[AAP instructions here]

# Lab 11: Copy boilerplate JCL to the PDSE, tailor it, and submit it

## Step 1: Copy boilerplate JCL to the PDSE 

Use module zos_copy as you did before, specifying member <HLQ>.LAB.JCL(TEXTOUT) as the destination for the copy. Don't delete or replace the entire PDSE. 

## Step 2: Tailor the boilerplate JCL

In a separate playbook, use module zos_replace in collection ibm_zos_core to replace the placeholder userid with your userid and to replace the original text, "OLD TEXT", with the value "NEW AND IMPROVED TEXT". 

## Step 3: Submit the job and display result

Extend the playbook you developed in Step 2 to submit the tailored JCL, check the maximum condition code, and either display the contents of SYSUT2 or show any errors that occurred.
...
#### ==========
#### Original labs below this point. 
#### ==========
